...
Looking for `{[bxcr]}` commands? See the [{{site.data.keyword.registryshort_notm}} CLI reference](/docs/services/Registry?topic=registry-registry_cli_reference#registry_cli_reference). Looking for `kubectl` commands? See the [Kubernetes documentation ![External link icon](../icons/launch-glyph.svg "External link icon")](https://kubernetes.io/docs/reference/kubectl/overview/).
{:tip}
...
The following beta versions of the redesigned {[pluginname]} are available. Note that the default behavior is `0.2`.

<table>
<caption>Beta versions of the redesigned {[pluginname]}</caption>
  <thead>
    <th>Beta version</th>
    <th>`{[bxcs]} help` output structure</th>
  ...
  <tbody>
    <tr>
      ...
      <td>Legacy and beta: You can run commands either in the legacy hyphenated structure (`{[bxcs]} alb-cert-get`) or in the beta spaced structure (`{[bxcs]} alb cert get`).</td>
  </tr>
    <tr>
      <td><code>0.3</code></td>
    ...
    <tr>
      <td><code>1.0</code></td>
      <td>Beta: Commands are shown in the spaced structure and are listed in categories.</td>
      <td>Beta: You can run commands only in the beta spaced structure (`{[bxcs]} alb cert get`).</td>
    </tr>
  </tbody>
</table>
...
**Tip:** To see the version of the {[pluginname]}, run the following command.
...
<col width="25%">
 ...
 <tbody>
  <tr>
    <td>[{[bxcs]} api](#cs_api)</td>
    <td>[{[bxcs]} api-key-info](#cs_api_key_info)</td>
    <td>[{[bxcs]} api-key-reset](#cs_api_key_reset)</td>
    <td>[{[bxcs]} apiserver-config-get](#cs_apiserver_config_get)</td>
  </tr>
  <tr>
    <td>[{[bxcs]} apiserver-config-set](#cs_apiserver_config_set)</td>
    <td>[{[bxcs]} apiserver-config-unset](#cs_apiserver_config_unset)</td>
    <td>[{[bxcs]} apiserver-refresh](#cs_apiserver_refresh)</td>
    <td></td>
 </tr>
</tbody>
</table>
...
<col width="25%">
 ...
 <tbody>
  <tr>
    <td>[{[bxcs]} help](#cs_help)</td>
    <td>[{[bxcs]} init](#cs_init)</td>
    <td>[{[bxcs]} messages](#cs_messages)</td>
    <td></td>
  </tr>
</tbody>
</table>
...
<col width="25%">
 ...
 <tbody>
  <tr>
    <td>[{[bxcs]} cluster-addon-disable](#cs_cluster_addon_disable)</td>
    <td>[{[bxcs]} cluster-addon-enable](#cs_cluster_addon_enable)</td>
    <td>[{[bxcs]} cluster-addons](#cs_cluster_addons)</td>
    <td>[{[bxcs]} cluster-config](#cs_cluster_config)</td>
  </tr>
  <tr>
    <td>[{[bxcs]} cluster-create](#cs_cluster_create)</td>
    <td>[{[bxcs]} cluster-feature-enable](#cs_cluster_feature_enable)</td>
    <td>[{[bxcs]} cluster-get](#cs_cluster_get)</td>
    <td>[{[bxcs]} cluster-pull-secret-apply](#cs_cluster_pull_secret_apply)</td>
  </tr>
  <tr>
    <td>[{[bxcs]} cluster-refresh](#cs_cluster_refresh)</td>
    <td>[{[bxcs]} cluster-rm](#cs_cluster_rm)</td>
    <td>[{[bxcs]} cluster-update](#cs_cluster_update)</td>
    <td>[{[bxcs]} clusters](#cs_clusters)</td>
  </tr>
  <tr>
    <td>[{[bxcs]} kube-versions](#cs_kube_versions)</td>
    <td> </td>
    <td> </td>
    <td> </td>
  </tr>
...
<col width="25%">
 ...
 <tbody>
    <tr>
      <td>Deprecated: [{[bxcs]} worker-add](#cs_worker_add)</td>
      <td>[{[bxcs]} worker-get](#cs_worker_get)</td>
      <td>[{[bxcs]} worker-reboot](#cs_worker_reboot)</td>
      <td>[{[bxcs]} worker-reload](#cs_worker_reload)</td>
    </tr>
    <tr>
      <td>[{[bxcs]} worker-rm](#cs_worker_rm)</td>
      <td>[{[bxcs]} worker-update](#cs_worker_update)</td>
      <td>[{[bxcs]} workers](#cs_workers)</td>
      <td> </td>
    </tr>
  </tbody>
</table>
...
<col width="25%">
 ...
 <tbody>
    <tr>
      <td>[{[bxcs]} worker-pool-create](#cs_worker_pool_create)</td>
      <td>[{[bxcs]} worker-pool-get](#cs_worker_pool_get)</td>
      <td>[{[bxcs]} worker-pool-rebalance](#cs_rebalance)</td>
      <td>[{[bxcs]} worker-pool-resize](#cs_worker_pool_resize)</td>
    </tr>
    <tr>
      <td>[{[bxcs]} worker-pool-rm](#cs_worker_pool_rm)</td>
      <td>[{[bxcs]} worker-pools](#cs_worker_pools)</td>
      <td>[{[bxcs]} zone-add](#cs_zone_add)</td>
      <td>[{[bxcs]} zone-network-set](#cs_zone_network_set)</td>
    </tr>
    <tr>
     <td>[{[bxcs]} zone-rm](#cs_zone_rm)</td>
     <td></td>
    </tr>
  </tbody>
</table>
...
```
{[bxcs]} api --endpoint ENDPOINT [--insecure] [--skip-ssl-validation] [--api-version VALUE] [-s]
```
{: pre}

Switching regions? Use the `{[bxcs]} region-set` [command](#cs_region-set) instead.
{: tip}
...
**Example**:
  ...
  ```
  {[bxcs]} api-key-info --cluster my_cluster
  ```
  {: pre}
...
```
{[bxcs]} api-key-reset [-s]
```
{: pre}

This command requires the {{site.data.keyword.containerlong_notm}} admin access policy and stores the API key of the user that executes this command in the account. The {{site.data.keyword.Bluemix_notm}} IAM API key is required to order infrastructure from the {[softlayer]} portfolio. Once stored, the API key is used for every action in a region that requires infrastructure permissions independent of the user that executes this command. For more information about how {{site.data.keyword.Bluemix_notm}} IAM API keys work, see the [`{[bxcs]} api-key-info` command](#cs_api_key_info).

Before you use this command, make sure that the user who executes this command has the required [{{site.data.keyword.containerlong_notm}} and {[softlayer]} permissions](/docs/containers?topic=containers-users#users).
{: important}
...
#### {[bxcs]} apiserver-config-get audit-webhook
...
```
{[bxcs]} apiserver-config-get audit-webhook --cluster CLUSTER
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} apiserver-config-get audit-webhook --cluster my_cluster
  ```
  {: pre}


### {[bxcs]} apiserver-config-set
...
#### {[bxcs]} apiserver-config-set audit-webhook
{: #cs_apiserver_set}
...
```
{[bxcs]} apiserver-config-set audit-webhook --cluster CLUSTER [--remoteServer SERVER_URL_OR_IP] [--caCert CA_CERT_PATH] [--clientCert CLIENT_CERT_PATH] [--clientKey CLIENT_KEY_PATH]
```
{: pre}

After you set the webhook, you must run the `{[bxcs]} apiserver-refresh` command to apply the changes to the Kubernetes master.
{: note}
...
**Example**:
  ...
  ```
  {[bxcs]} apiserver-config-set audit-webhook --cluster my_cluster --remoteServer https://audit.example.com/audit --caCert /mnt/etc/kubernetes/apiserver-audit/ca.pem --clientCert /mnt/etc/kubernetes/apiserver-audit/cert.pem --clientKey /mnt/etc/kubernetes/apiserver-audit/key.pem
  ```
  {: pre}


### {[bxcs]} apiserver-config-unset
...
#### {[bxcs]} apiserver-config-unset audit-webhook
{: #cs_apiserver_unset}
...
```
{[bxcs]} apiserver-config-unset audit-webhook --cluster CLUSTER
```
{: pre}

After you disable the webhook, you must run the `{[bxcs]} apiserver-refresh` command to apply the changes to the Kubernetes master.
{: note}
...
**Example**:
  ...
  ```
  {[bxcs]} apiserver-refresh --cluster my_cluster
  ```
  {: pre}
...
Initialize the {[pluginname]} or specify the region where you want to create or access Kubernetes clusters.
{: shortdesc}

```
{[bxcs]} init [--host HOST] [--insecure] [-p] [-u] [-s]
```
{: pre}
...
**Example**:
```
{[bxcs]} init --host https://uk-south.containers.cloud.ibm.com
```
{: pre}
...
#### {[bxcs]} cluster-addon-disable istio
...
```
{[bxcs]} cluster-addon-disable istio --cluster CLUSTER [-f]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-addon-disable istio --cluster my_cluster
  ```
  {: pre}

#### {[bxcs]} cluster-addon-disable istio-extras
...
```
{[bxcs]} cluster-addon-disable istio-extras --cluster CLUSTER [-f]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-addon-disable istio-extras --cluster my_cluster
  ```
  {: pre}

#### {[bxcs]} cluster-addon-disable istio-sample-bookinfo
...
```
{[bxcs]} cluster-addon-disable istio-sample-bookinfo --cluster CLUSTER
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-addon-disable istio-sample-bookinfo --cluster my_cluster
  ```
  {: pre}

#### {[bxcs]} cluster-addon-disable knative
...
```
{[bxcs]} cluster-addon-disable knative --cluster CLUSTER
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-addon-disable knative --cluster my_cluster
  ```
  {: pre}

### {[bxcs]} cluster-addon-enable
...
#### {[bxcs]} cluster-addon-enable istio
...
```
{[bxcs]} cluster-addon-enable istio --cluster CLUSTER
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-addon-enable istio --cluster my_cluster
  ```
  {: pre}

#### {[bxcs]} cluster-addon-enable istio-extras
...
```
{[bxcs]} cluster-addon-enable istio-extras --cluster CLUSTER [-y]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-addon-enable istio-extras --cluster my_cluster
  ```
  {: pre}

#### {[bxcs]} cluster-addon-enable istio-sample-bookinfo
...
```
{[bxcs]} cluster-addon-enable istio-sample-bookinfo --cluster CLUSTER [-y]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-addon-enable istio-sample-bookinfo --cluster my_cluster
  ```
  {: pre}

#### {[bxcs]} cluster-addon-enable knative
...
```
{[bxcs]} cluster-addon-enable knative --cluster CLUSTER [-y]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-addon-enable knative --cluster my_cluster
  ```
  {: pre}
...
```
{[bxcs]} cluster-addons --cluster CLUSTER
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-addons --cluster my_cluster
  ```
  {: pre}
...
After logging in, download Kubernetes configuration data and certificates to connect to your cluster and run `kubectl` commands. The files are downloaded to `user_home_directory/.bluemix/plugins/{[pluginname_short]}/clusters/<cluster_name>`.
{: shortdesc}

```
{[bxcs]} cluster-config --cluster CLUSTER [--admin] [--export] [--network] [--skip-rbac] [-s] [--yaml]
```
{: pre}
...
**Command options**:
   ...
   <dd>Download the TLS certificates and permission files for the Super User role. You can use the certs to automate tasks in a cluster without having to re-authenticate. The files are downloaded to `<user_home_directory>/.bluemix/plugins/{[pluginname_short]}/clusters/<cluster_name>-admin`. This value is optional.</dd>
...
```
{[bxcs]} cluster-config --cluster my_cluster
```
{: pre}
...
```
{[bxcs]} cluster-create [--file FILE_LOCATION] [--hardware HARDWARE] --zone ZONE --machine-type MACHINE_TYPE --name NAME [--kube-version MAJOR.MINOR.PATCH] [--no-subnet] [--private-vlan PRIVATE_VLAN] [--public-vlan PUBLIC_VLAN] [--private-only] [--workers WORKER] [--disable-disk-encrypt] [--trusted] [-s]
```
{: pre}
...
<table>
    ...
    <td>Replace <code><em>&lt;cluster_name&gt;</em></code> with a name for your cluster. {[cluster-name.md]}</td>
    </tr>
    <tr>
    <td><code><em>zone</em></code></td>
    <td>Replace <code><em>&lt;zone&gt;</em></code> with the zone where you want to create your cluster. The available zones are dependent on the region that you are logged in. To list available zones, run <code>{[bxcs]} zones</code>. </td>
     </tr>
     <tr>
     ...
     <td>Replace <code><em>&lt;machine_type&gt;</em></code> with the type of machine that you want to deploy your worker nodes to. You can deploy your worker nodes as virtual machines on shared or dedicated hardware, or as physical machines on bare metal. Available physical and virtual machines types vary by the zone in which you deploy the cluster. For more information, see the documentation for the `{[bxcs]} machine-type` [command](/docs/containers?topic=containers-cs_cli_reference#cs_machine_types).</td>
     </tr>
     <tr>
     <td><code><em>private-vlan</em></code></td>
     <td>Replace <code><em>&lt;private_VLAN&gt;</em></code> with the ID of the private VLAN that you want to use for your worker nodes. To list available VLANs, run <code>{[bxcs]} vlans <em>&lt;zone&gt;</em></code> and look for VLAN routers that start with <code>bcr</code> (back-end router).</td>
     </tr>
     <tr>
     <td><code><em>public-vlan</em></code></td>
     <td>Replace <code><em>&lt;public_VLAN&gt;</em></code> with the ID of the public VLAN that you want to use for your worker nodes. To list available VLANs, run <code>{[bxcs]} vlans <em>&lt;zone&gt;</em></code> and look for VLAN routers that start with <code>fcr</code> (front-end router).</td>
     </tr>
     <tr>
...
<dd>Choose a machine type. You can deploy your worker nodes as virtual machines on shared or dedicated hardware, or as physical machines on bare metal. Available physical and virtual machines types vary by the zone in which you deploy the cluster. For more information, see the documentation for the `{[bxcs]} machine-types` [command](/docs/containers?topic=containers-cs_cli_reference#cs_machine_types). This value is required for standard clusters and is not available for free clusters.</dd>
...
<li>If you created a standard cluster before in this zone or created a private VLAN in {[softlayer]} before, you must specify that private VLAN. {[matching_vlans]}</li>
</ul>

<p>To find out if you already have a private VLAN for a specific zone or to find the name of an existing private VLAN, run <code>{[bxcs]} vlans <em>&lt;zone&gt;</em></code>.</p></dd>
...
<li>If you created a standard cluster before in this zone or created a public VLAN in {[softlayer]} before, specify that public VLAN. If you want to connect your worker nodes to a private VLAN only, do not specify this option. {[matching_vlans]}</li>
</ul>

<p>To find out if you already have a public VLAN for a specific zone or to find the name of an existing public VLAN, run <code>{[bxcs]} vlans <em>&lt;zone&gt;</em></code>.</p></dd>
...
<p>To check whether the bare metal machine type supports trust, check the `Trustable` field in the output of the `{[bxcs]} machine-types <zone>` [command](#cs_machine_types). To verify that a cluster is trust-enabled, view the **Trust ready** field in the output of the `{[bxcs]} cluster-get` [command](#cs_cluster_get). To verify a bare metal worker node is trust-enabled, view the **Trust** field in the output of the `{[bxcs]} worker-get` [command](#cs_worker_get).</p></dd>
...
**Examples**:
  ...
  **Create your first standard cluster**: The first standard cluster that is created in a zone also creates a private VLAN. Therefore, do not include the `--public-vlan` flag.
  ...
  ```
  {[bxcs]} cluster-create --zone dal10 --private-vlan my_private_VLAN_ID --machine-type b2c.4x16 --name my_cluster --hardware shared --workers 2
  ```
  {: pre}

  **Create subsequent standard clusters**: If you already created a standard cluster in this zone or created a public VLAN in {[softlayer]} before, specify that public VLAN with the `--public-vlan` flag. To find out if you already have a public VLAN for a specific zone or to find the name of an existing public VLAN, run `{[bxcs]} vlans <zone>`.

  ```
  {[bxcs]} cluster-create --zone dal10 --public-vlan my_public_VLAN_ID --private-vlan my_private_VLAN_ID --machine-type b2c.4x16 --name my_cluster --hardware shared --workers 2
  ```
  {: pre}
  ...
  **Create a cluster in an {{site.data.keyword.Bluemix_dedicated_notm}} environment**:
  ...
  ```
  {[bxcs]} cluster-create --machine-type machine-type --workers number --name cluster_name
  ```
  {: pre}


### {[bxcs]} cluster-feature-enable
...
```
{[bxcs]} cluster-feature-enable [-f] --cluster CLUSTER [--trusted] [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-feature-enable --cluster my_cluster --trusted=true
  ```
  {: pre}
...
```
{[bxcs]} cluster-get --cluster CLUSTER [--json] [--showResources] [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-get --cluster my_cluster --showResources
  ```
  {: pre}
...
Location:                       dal10
Master URL:                     https://{[public_DNS]}:30426
...
### {[bxcs]} cluster-pull-secret-apply
...
```
{[bxcs]} cluster-pull-secret-apply --cluster CLUSTER
```
{: pre}
...
```
{[bxcs]} cluster-pull-secret-apply --cluster my_cluster
```
{: pre}

### {[bxcs]} cluster-refresh
{: #cs_cluster_refresh}
...
```
{[bxcs]} cluster-refresh --cluster CLUSTER [-f] [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-refresh --cluster my_cluster
  ```
  {: pre}
...
```
{[bxcs]} cluster-rm --cluster CLUSTER [--force-delete-storage] [-f] [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-rm --cluster my_cluster
  ```
  {: pre}
...
```
{[bxcs]} cluster-update --cluster CLUSTER [--kube-version MAJOR.MINOR.PATCH] [--force-update] [-f] [-s]
```
{: pre}
...
<strong>Command options</strong>:
   ...
   <dt><code>--kube-version <em>MAJOR.MINOR.PATCH</em></code></dt>
   <dd>The Kubernetes version of the cluster. If you do not specify a version, the Kubernetes master is updated to the default API version. To see available versions, run [{[bxcs]} kube-versions](#cs_kube_versions). This value is optional.</dd>
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-update --cluster my_cluster
  ```
  {: pre}
...
```
{[bxcs]} cluster-service-bind --cluster CLUSTER --namespace KUBERNETES_NAMESPACE --service SERVICE_INSTANCE_GUID [--key SERVICE_INSTANCE_KEY] [--role IAM_ROLE] [-s]
```
{: pre}
...
<strong>Command options</strong>:
   ...
   <dd>The ID of the {{site.data.keyword.Bluemix_notm}} service instance that you want to bind. To find the ID, run <code>{[bx]} service show <service_instance_name> --guid</code>. This value is required. </dd>
   ...
   To list available roles for the service, run `{[bx]} iam roles --service <service_name>`. The service name is the name of the service in the catalog which you can get by running `{[bx]} catalog search`.</dd>
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-service-bind --cluster my_cluster --namespace my_namespace --service my_service_instance
  ```
  {: pre}


### {[bxcs]} cluster-service-unbind
...
```
{[bxcs]} cluster-service-unbind --cluster CLUSTER --namespace KUBERNETES_NAMESPACE --service SERVICE_INSTANCE_GUID [-s]
```
{: pre}
...
<strong>Command options</strong>:
   ...
   <dt><code>--service <em>SERVICE_INSTANCE_GUID</em></code></dt>
   <dd>The ID of the {{site.data.keyword.Bluemix_notm}} service instance that you want to remove. To find the ID of the service instance, run `{[bxcs]} cluster-services <cluster_name_or_ID>`. This value is required.</dd>
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-service-unbind --cluster my_cluster --namespace my_namespace --service 8567221
  ```
  {: pre}


### {[bxcs]} cluster-services
{: #cs_cluster_services}
...
```
{[bxcs]} cluster-services --cluster CLUSTER [--namespace KUBERNETES_NAMESPACE] [--all-namespaces] [--json] [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-services --cluster my_cluster --namespace my_namespace
  ```
  {: pre}
...
```
{[bxcs]} va --container CONTAINER_ID [--extended] [--vulnerabilities] [--configuration-issues] [--json]
```
{: pre}
...
<p>To find the ID of your container:<ol><li>[Target the Kubernetes CLI to your cluster](/docs/containers?topic=containers-cs_cli_install#cs_cli_configure).</li><li>List your pods by running `kubectl get pods`.</li><li>Find the **Container ID** field in the output of the `kubectl describe pod <pod_name>` command. For example, `Container ID: containerd://1a11a1aa2b2b22223333c44444ccc555667d7dd777888e8ef99f1011121314g15`.</li><li>Remove the `containerd://` prefix from the ID before you use the container ID for the `{[bxcs]} va` command. For example, `1a11a1aa2b2b22223333c44444ccc555667d7dd777888e8ef99f1011121314g15`.</li></ol></p></dd>
...
```
{[bxcs]} va --container 1a11a1aa2b2b22223333c44444ccc555667d7dd777888e8ef99f1011121314g15 --extended --vulnerabilities --json
```
{: pre}

### {[bxcs]} key-protect-enable
{: #cs_key_protect}
...
```
{[bxcs]} key-protect-enable --cluster CLUSTER_NAME_OR_ID --key-protect-url ENDPOINT --key-protect-instance INSTANCE_GUID --crk ROOT_KEY_ID
```
{: pre}
...
<dd>Your {{site.data.keyword.keymanagementserviceshort}} instance GUID. To get the instance GUID, run <code>{[bx]} resource service-instance SERVICE_INSTANCE_NAME --id</code> and copy the second value (not the full CRN).</dd>
...
```
{[bxcs]} key-protect-enable --cluster mycluster --key-protect-url keyprotect.us-south.bluemix.net --key-protect-instance a11aa11a-bbb2-3333-d444-e5e555e5ee5 --crk 1a111a1a-bb22-3c3c-4d44-55e555e55e55
```
{: pre}
...
```
{[bxcs]} webhook-create --cluster CLUSTER --level LEVEL --type slack --url URL  [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} webhook-create --cluster my_cluster --level Normal --type slack --url http://github.com/mywebhook
  ```
  {: pre}
...
### {[bxcs]} cluster-subnet-add
{: #cs_cluster_subnet_add}

You can add existing portable public or private subnets from your {[softlayer]} account to your Kubernetes cluster or reuse subnets from a deleted cluster instead of using the automatically provisioned subnets.
{: shortdesc}

```
{[bxcs]} cluster-subnet-add --cluster CLUSTER --subnet-id SUBNET [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-subnet-add --cluster my_cluster --subnet-id 1643389
  ```
  {: pre}


### {[bxcs]} cluster-subnet-create
{: #cs_cluster_subnet_create}

Create a subnet in an {[softlayer]} account and make it available to a specified cluster in {{site.data.keyword.containerlong_notm}}.
{: shortdesc}

```
{[bxcs]} cluster-subnet-create --cluster CLUSTER --size SIZE --vlan VLAN_ID [-s]
```
{: pre}
...
<strong>Command options</strong>:
   ...
   <dl>
   <dt><code>--cluster <em>CLUSTER</em></code></dt>
   <dd>The name or ID of the cluster. This value is required. To list your clusters, use the `{[bxcs]} clusters` [command](#cs_clusters).</dd>
   ...
   <dd>The VLAN in which to create the subnet. This value is required. To list available VLANS, use the `{[bxcs]} vlans <zone>` [command](#cs_vlans). The subnet is provisioned in the same zone that the VLAN is in.</dd>
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-subnet-create --cluster my_cluster --size 8 --vlan 1764905
  ```
  {: pre}


### {[bxcs]} cluster-user-subnet-add
...
```
{[bxcs]} cluster-user-subnet-add --cluster CLUSTER --subnet-cidr SUBNET_CIDR --private-vlan PRIVATE_VLAN
```
{: pre}

This private subnet is not one provided by {[softlayer]}. As such, you must configure any inbound and outbound network traffic routing for the subnet. To add an {[softlayer]} subnet, use the `{[bxcs]} cluster-subnet-add` [command](#cs_cluster_subnet_add).
...
<strong>Command options</strong>:
   ...
   <dd>The subnet Classless InterDomain Routing (CIDR). This value is required, and must not conflict with any subnet that is used by {[softlayer]}.
...
**Example**:
  ...
  ```
  {[bxcs]} cluster-user-subnet-add --cluster my_cluster --subnet-cidr {[public_IP]}/29 --private-vlan 1502175
  ```
  {: pre}


### {[bxcs]} cluster-user-subnet-rm
...
```
{[bxcs]} cluster-user-subnet-rm --cluster CLUSTER --subnet-cidr SUBNET_CIDR --private-vlan PRIVATE_VLAN
```
{: pre}
...
<strong>Command options</strong>:
   ...
   <dt><code>--subnet-cidr <em>SUBNET_CIDR</em></code></dt>
   <dd>The subnet Classless InterDomain Routing (CIDR). This value is required, and must match the CIDR that was set by the `{[bxcs]} cluster-user-subnet-add` [command](#cs_cluster_user_subnet_add).</dd>

   <dt><code>--private-vlan <em>PRIVATE_VLAN</em></code></dt>
   <dd>The ID of the private VLAN. This value is required, and must match the VLAN ID that was set by the `{[bxcs]} cluster-user-subnet-add` [command](#cs_cluster_user_subnet_add).</dd>
   </dl>

**Example**:

  ```
  {[bxcs]} cluster-user-subnet-rm --cluster my_cluster --subnet-cidr {[public_IP]}/29 --private-vlan 1502175
  ```
  {: pre}
...
View a list of subnets that are available in an {[softlayer]} account.
{: shortdesc}
...
```
{[bxcs]} alb-autoupdate-disable --cluster mycluster
```
{: pre}

### {[bxcs]} alb-autoupdate-enable
...
```
{[bxcs]} alb-autoupdate-enable --cluster CLUSTER
```
{: pre}
...
```
{[bxcs]} alb-autoupdate-enable --cluster mycluster
```
{: pre}

### {[bxcs]} alb-autoupdate-get
...
```
{[bxcs]} alb-autoupdate-get --cluster CLUSTER
```
{: pre}
...
```
{[bxcs]} alb-autoupdate-get --cluster mycluster
```
{: pre}

### {[bxcs]} alb-cert-deploy
{: #cs_alb_cert_deploy}
...
```
{[bxcs]} alb-cert-deploy [--update] --cluster CLUSTER --secret-name SECRET_NAME --cert-crn CERTIFICATE_CRN [--update] [-s]
```
{: pre}
...
Example for deploying an ALB secret:
   ...
   ```
   {[bxcs]} alb-cert-deploy --secret-name my_alb_secret --cluster my_cluster --cert-crn crn:v1:staging:public:cloudcerts:us-south:a/06580c923e40314421d3b6cb40c01c68:0db4351b-0ee1-479d-af37-56a4da9ef30f:certificate:4bc35b7e0badb304e60aef00947ae7ff
   ```
   {: pre}
...
Example for updating an existing ALB secret:
 ...
 ```
 {[bxcs]} alb-cert-deploy --update --secret-name my_alb_secret --cluster my_cluster --cert-crn crn:v1:staging:public:cloudcerts:us-south:a/06580c923e40314421d3b6cb40c01c68:0db4351b-0ee1-479d-af37-56a4da9ef30f:certificate:7e21fde8ee84a96d29240327daee3eb2
 ```
 {: pre}
...
```
{[bxcs]} alb-cert-get --cluster CLUSTER [--secret-name SECRET_NAME] [--cert-crn CERTIFICATE_CRN] [--json] [-s]
```
{: pre}
...
**Examples**:
 ...
 ```
 {[bxcs]} alb-cert-get --cluster my_cluster --secret-name my_alb_secret
 ```
 {: pre}
 ...
 ```
 {[bxcs]} alb-cert-get --cluster my_cluster --cert-crn  crn:v1:staging:public:cloudcerts:us-south:a/06580c923e40314421d3b6cb40c01c68:0db4351b-0ee1-479d-af37-56a4da9ef30f:certificate:4bc35b7e0badb304e60aef00947ae7ff
 ```
 {: pre}
...
```
{[bxcs]} alb-cert-rm --cluster CLUSTER [--secret-name SECRET_NAME] [--cert-crn CERTIFICATE_CRN] [-s]
```
{: pre}
...
**Examples**:
 ...
 ```
 {[bxcs]} alb-cert-rm --cluster my_cluster --secret-name my_alb_secret
 ```
 {: pre}
 ...
 ```
 {[bxcs]} alb-cert-rm --cluster my_cluster --cert-crn crn:v1:staging:public:cloudcerts:us-south:a/06580c923e40314421d3b6cb40c01c68:0db4351b-0ee1-479d-af37-56a4da9ef30f:certificate:4bc35b7e0badb304e60aef00947ae7ff
 ```
 {: pre}
...
```
{[bxcs]} alb-certs --cluster CLUSTER [--json] [-s]
```
{: pre}
...
**Example**:
 ...
 ```
 {[bxcs]} alb-certs --cluster my_cluster
 ```
 {: pre}
...
```
{[bxcs]} alb-configure --albID ALB_ID [--enable] [--user-ip USER_IP] [--disable] [--disable-deployment] [-s]
```
{: pre}
...
**Command options**:
   ...
   <dd>The ID for an ALB. Run <code>{[bxcs]} albs <em>--cluster </em>CLUSTER</code> to view the IDs for the ALBs in a cluster. This value is required.</dd>
...
**Examples**:
  ...
  ```
  {[bxcs]} alb-configure --albID private-cr18a61a63a6a94b658596aa93a087aaa9-alb1 --enable
  ```
  {: pre}
  ...
  ```
  {[bxcs]} alb-configure --albID private-cr18a61a63a6a94b658596aa93a087aaa9-alb1 --enable --user-ip user_ip
  ```
  {: pre}
  ...
  ```
  {[bxcs]} alb-configure --albID public-cr18a61a63a6a94b658596aa93a087aaa9-alb1 --disable
  ```
  {: pre}
...
```
{[bxcs]} alb-get --albID ALB_ID [--json] [-s]
```
{: pre}
...
<strong>Command options</strong>:
   ...
   <dl>
   <dt><code><em>--albID </em>ALB_ID</code></dt>
   <dd>The ID for an ALB. Run <code>{[bxcs]} albs --cluster <em>CLUSTER</em></code> to view the IDs for the ALBs in a cluster. This value is required.</dd>
...
**Example**:
  ...
  ```
  {[bxcs]} alb-get --albID public-cr18a61a63a6a94b658596aa93a087aaa9-alb1
  ```
  {: pre}
...
```
{[bxcs]} alb-rollback --cluster CLUSTER
```
{: pre}
...
```
{[bxcs]} alb-rollback --cluster mycluster
```
{: pre}
...
```
{[bxcs]} alb-update --cluster CLUSTER
```
{: pre}
...
```
{[bxcs]} alb-update --cluster <cluster_name_or_ID>
```
{: pre}
...
```
{[bxcs]} albs --cluster CLUSTER [--json] [-s]
```
{: pre}
...
Set {[softlayer]} account credentials for an {{site.data.keyword.containerlong_notm}} resource group and region.
{: shortdesc}

```
{[bxcs]} credential-set --infrastructure-api-key API_KEY --infrastructure-username USERNAME [-s]
```
{: pre}

If you have an {{site.data.keyword.Bluemix_notm}} Pay-As-You-Go account, you have access to the {[softlayer]} portfolio by default. However, you might want to use a different {[softlayer]} account that you already have to order infrastructure. You can link this infrastructure account to your {{site.data.keyword.Bluemix_notm}} account by using this command.

If {[softlayer]} credentials are manually set for a region and a resource group, these credentials are used to order infrastructure for all clusters within that region in the resource group. These credentials are used to determine infrastructure permissions, even if an [{{site.data.keyword.Bluemix_notm}} IAM API key](#cs_api_key_info) already exists for the resource group and region. If the user whose credentials are stored does not have the required permissions to order infrastructure, then infrastructure-related actions, such as creating a cluster or reloading a worker node can fail.

You cannot set multiple credentials for the same {{site.data.keyword.containerlong_notm}} resource group and region.

Before you use this command, make sure that the user whose credentials are used has the required [{{site.data.keyword.containerlong_notm}} and {[softlayer]} permissions](/docs/containers?topic=containers-users#users).
{: important}
...
<strong>Command options</strong>:
   ...
   <dd>{[softlayer]} account API username. This value is required. Note that the infrastructure API user name is not the same as the IBMid. To view the infrastructure API user name:
   <ol>
   ...
   <dd>{[softlayer]} account API key. This value is required. To view or generate an infrastructure API key:
  <li>Log in to the [{{site.data.keyword.Bluemix_notm}} console ![External link icon](../icons/launch-glyph.svg "External link icon")](https://cloud.ibm.com).
  <li>From the menu bar, select **Manage > Access (IAM)**.
  <li>Select the **Users** tab and then click on your user name.
...
**Example**:
  ...
  ```
  {[bxcs]} credential-set --infrastructure-api-key <api_key> --infrastructure-username dbmanager
  ```
  {: pre}


### {[bxcs]} credential-unset
{: #cs_credentials_unset}

Remove {[softlayer]} account credentials from an {{site.data.keyword.containerlong_notm}} region.
{: shortdesc}
...
After you remove the credentials, the [{{site.data.keyword.Bluemix_notm}} IAM API key](#cs_api_key_info) is used to order resources in {[softlayer]}.
...
```
{[bxcs]} machine-types --zone ZONE [--json] [-s]
```
{: pre}
...
List the public and private VLANs that are available for a zone in your {[softlayer]} account. To list available VLANs, you must have a paid account.
{: shortdesc}

```
{[bxcs]} vlans --zone ZONE [--all] [--json] [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} vlans --zone dal10
  ```
  {: pre}


### {[bxcs]} vlan-spanning-get
{: #cs_vlan_spanning_get}

View the VLAN spanning status for an {[softlayer]} account. VLAN spanning enables all devices on an account to communicate with each other by means of the private network, regardless of its assigned VLAN.
{: shortdesc}

```
{[bxcs]} vlan-spanning-get [--json] [-s]
```
{: pre}
...
### {[bxcs]} logging-autoupdate-disable
...
```
{[bxcs]} logging-autoupdate-disable --cluster CLUSTER
```
{: pre}
...
### {[bxcs]} logging-autoupdate-enable
...
```
{[bxcs]} logging-autoupdate-enable --cluster CLUSTER
```
{: pre}
...
```
{[bxcs]} logging-autoupdate-get --cluster CLUSTER
```
{: pre}
...
```
{[bxcs]} logging-config-create --cluster CLUSTER --logsource LOG_SOURCE --type LOG_TYPE [--namespace KUBERNETES_NAMESPACE] [--hostname LOG_SERVER_HOSTNAME_OR_IP] [--port LOG_SERVER_PORT] [--space CLUSTER_SPACE] [--org CLUSTER_ORG] [--app-containers CONTAINERS] [--app-paths PATHS_TO_LOGS] [--syslog-protocol PROTOCOL]  [--json] [--skip-validation] [--force-update][-s]
```
{: pre}
...
Example for log type `ibm` that forwards from a `container` log source on default port 9091:
  ...
  ```
  {[bxcs]} logging-config-create my_cluster --logsource container --namespace my_namespace --hostname ingest.logging.ng.bluemix.net --type ibm
  ```
  {: pre}
...
Example for log type `syslog` that forwards from a `container` log source on default port 514:
  ...
  ```
  {[bxcs]} logging-config-create my_cluster --logsource container --namespace my_namespace  --hostname {[public_IP]} --type syslog
  ```
  {: pre}
...
Example for log type `syslog` that forwards logs from an `ingress` source on a port different than the default:
  ...
  ```
  {[bxcs]} logging-config-create --cluster my_cluster --logsource container --hostname {[public_IP]} --port 5514 --type syslog
  ```
  {: pre}


### {[bxcs]} logging-config-get
{: #cs_logging_get}
...
```
{[bxcs]} logging-config-get --cluster CLUSTER [--logsource LOG_SOURCE] [--json] [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} logging-config-get --cluster my_cluster --logsource worker
  ```
  {: pre}


### {[bxcs]} logging-config-refresh
{: #cs_logging_refresh}
...
```
{[bxcs]} logging-config-refresh --cluster CLUSTER [--force-update] [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} logging-config-refresh --cluster my_cluster
  ```
  {: pre}



### {[bxcs]} logging-config-rm
{: #cs_logging_rm}
...
```
{[bxcs]} logging-config-rm --cluster CLUSTER [--id LOG_CONFIG_ID] [--all] [--force-update] [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} logging-config-rm --cluster my_cluster --id f4bc77c0-ee7d-422d-aabf-a4e6b977264e
  ```
  {: pre}


### {[bxcs]} logging-config-update
{: #cs_logging_update}
...
```
{[bxcs]} logging-config-update --cluster CLUSTER --id LOG_CONFIG_ID --type LOG_TYPE  [--namespace NAMESPACE] [--hostname LOG_SERVER_HOSTNAME_OR_IP] [--port LOG_SERVER_PORT] [--space CLUSTER_SPACE] [--org CLUSTER_ORG] [--app-paths PATH] [--app-containers PATH] [--json] [--skipValidation] [--force-update] [-s]
```
{: pre}
...
**Example for log type `ibm`**:
  ...
  ```
  {[bxcs]} logging-config-update my_cluster --id f4bc77c0-ee7d-422d-aabf-a4e6b977264e --type ibm
  ```
  {: pre}
...
**Example for log type `syslog`**:
  ...
  ```
  {[bxcs]} logging-config-update --cluster my_cluster --id f4bc77c0-ee7d-422d-aabf-a4e6b977264e --hostname localhost --port 5514 --type syslog
  ```
  {: pre}



### {[bxcs]} logging-filter-create
...
```
{[bxcs]} logging-filter-create --cluster CLUSTER --type LOG_TYPE [--logging-configs CONFIGS] [--namespace KUBERNETES_NAMESPACE] [--container CONTAINER_NAME] [--level LOGGING_LEVEL] [--message MESSAGE] [--regex-message MESSAGE] [--force-update] [--json] [-s]
```
{: pre}
...
This example filters out all logs that are forwarded from containers with the name `test-container` in the default namespace that are at the debug level or less, and have a log message that contains "GET request".
  ...
  ```
  {[bxcs]} logging-filter-create --cluster example-cluster --type container --namespace default --container test-container --level debug --message "GET request"
  ```
  {: pre}
...
This example filters out all of the logs that are forwarded, at an info level or less, from a specific cluster. The output is returned as JSON.
  ...
  ```
  {[bxcs]} logging-filter-create --cluster example-cluster --type all --level info --json
  ```
  {: pre}



### {[bxcs]} logging-filter-get
{: #cs_log_filter_view}
...
```
{[bxcs]} logging-filter-get --cluster CLUSTER [--id FILTER_ID] [--show-matching-configs] [--show-covering-filters] [--json] [-s]
```
{: pre}
...
```
{[bxcs]} logging-filter-get mycluster --id 885732 --show-matching-configs
```
{: pre}

### {[bxcs]} logging-filter-rm
...
```
{[bxcs]} logging-filter-rm --cluster CLUSTER [--id FILTER_ID] [--all] [--force-update] [-s]
```
{: pre}
...
```
{[bxcs]} logging-filter-rm mycluster --id 885732
```
{: pre}

### {[bxcs]} logging-filter-update
...
```
{[bxcs]} logging-filter-update --cluster CLUSTER --id FILTER_ID --type LOG_TYPE [--logging-configs CONFIGS] [--namespace KUBERNETES_NAMESPACE] [--container CONTAINER_NAME] [--level LOGGING_LEVEL] [--message MESSAGE] [--regex-message MESSAGE] [--force-update] [--json] [-s]
```
{: pre}
...
This example filters out all logs that are forwarded from containers with the name `test-container` in the default namespace that are at the debug level or less, and have a log message that contains "GET request".
  ...
  ```
  {[bxcs]} logging-filter-update --cluster example-cluster --id 885274 --type container --namespace default --container test-container --level debug --message "GET request"
  ```
  {: pre}
...
This example filters out all of the logs that are forwarded, at an info level or less, from a specific cluster. The output is returned as JSON.
  ...
  ```
  {[bxcs]} logging-filter-update --cluster example-cluster --id 274885 --type all --level info --json
  ```
  {: pre}

### {[bxcs]} logging-collect
{: #cs_log_collect}
...
```
{[bxcs]} logging-collect --cluster CLUSTER --cos-bucket BUCKET_NAME --cos-endpoint ENDPOINT --hmac-key-id HMAC_KEY_ID --hmac-key HMAC_KEY --type LOG_TYPE [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} logging-collect --cluster mycluster --cos-bucket mybucket --cos-endpoint s3-api.us-geo.objectstorage.softlayer.net --hmac-key-id e2e7f5c9fo0144563c418dlhi3545m86 --hmac-key c485b9b9fo4376722f692b63743e65e1705301ab051em96j
  ```
  {: pre}
...
**Example output**:
  ...
  OK
  The log collection request was successfully submitted. To view the status of the request run {[bxcs]} logging-collect-status mycluster.
  ```
  {: screen}


### {[bxcs]} logging-collect-status
...
```
{[bxcs]} logging-collect-status --cluster CLUSTER [--json] [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} logging-collect-status --cluster mycluster
  ```
  {: pre}
...
Find the {{site.data.keyword.containerlong_notm}} region that you are currently in. You create and manage clusters specific to the region. Use the `{[bxcs]} region-set` command to change regions.
{: shortdesc}
...
```
{[bxcs]} region-set [--region REGION]
```
{: pre}

For example, you can log in to {{site.data.keyword.Bluemix_notm}} in the US South region and create a cluster. Next, you can use `{[bxcs]} region-set eu-central` to target the EU Central region and create another cluster. Finally, you can use `{[bxcs]} region-set us-south` to return to US South to manage your cluster in that region.
...
For a list of available regions, review [regions and zones](/docs/containers?topic=containers-regions-and-zones) or use the `{[bxcs]} regions` [command](#cs_regions).</dd></dl>

**Example**:

```
{[bxcs]} region-set eu-central
```
{: pre}
...
View a list of available zones for you to create a cluster in. The available zones vary by the region that you are logged in to. To switch regions, run `{[bxcs]} region-set`.
{: shortdesc}

```
{[bxcs]} zones [--region-only] [--json] [-s]
```
{: pre}
...
```
{[bxcs]} worker-add --cluster CLUSTER [--file FILE_LOCATION] [--hardware HARDWARE] --machine-type MACHINE_TYPE --workers NUMBER --private-vlan PRIVATE_VLAN --public-vlan PUBLIC_VLAN [--disable-disk-encrypt] [-s]
```
{: pre}
...
<tr>
<td><code><em>zone</em></code></td>
<td>Replace <code><em>&lt;zone&gt;</em></code> with the zone to deploy your worker nodes. The available zones are dependent on the region that you are logged in. To list available zones, run <code>{[bxcs]} zones</code>.</td>
</tr>
<tr>
<td><code><em>machine-type</em></code></td>
<td>Replace <code><em>&lt;machine_type&gt;</em></code> with the type of machine that you want to deploy your worker nodes to. You can deploy your worker nodes as virtual machines on shared or dedicated hardware, or as physical machines on bare metal. Available physical and virtual machines types vary by the zone in which you deploy the cluster. For more information, see the `{[bxcs]} machine-types` [command](/docs/containers?topic=containers-cs_cli_reference#cs_machine_types).</td>
</tr>
<tr>
<td><code><em>private-vlan</em></code></td>
<td>Replace <code><em>&lt;private_VLAN&gt;</em></code> with the ID of the private VLAN that you want to use for your worker nodes. To list available VLANs, run <code>{[bxcs]} vlans <em>&lt;zone&gt;</em></code> and look for VLAN routers that start with <code>bcr</code> (back-end router).</td>
...
<td>Replace <code>&lt;public_VLAN&gt;</code> with the ID of the public VLAN that you want to use for your worker nodes. To list available VLANs, run <code>{[bxcs]} vlans &lt;zone&gt;</code> and look for VLAN routers that start with <code>fcr</code> (front-end router). <br><strong>Note</strong>: {[private_vlan_vyatta]}</td>
</tr>
<tr>
...
<dt><code>--private-vlan <em>PRIVATE_VLAN</em></code></dt>
<dd>The private VLAN that was specified when the cluster was created. This value is required. {[matching_vlans]}</dd>

<dt><code>--public-vlan <em>PUBLIC_VLAN</em></code></dt>
<dd>The public VLAN that was specified when the cluster was created. This value is optional. If you want your worker nodes to exist on a private VLAN only, do not provide a public VLAN ID. {[matching_vlans]}<p class="note">{[private_vlan_vyatta]}</p></dd>
...
**Examples**:
  ...
  ```
  {[bxcs]} worker-add --cluster my_cluster --workers 3 --public-vlan my_public_VLAN_ID --private-vlan my_private_VLAN_ID --machine-type b2c.4x16 --hardware shared
  ```
  {: pre}
  ...
  ```
  {[bxcs]} worker-add --cluster my_cluster --workers 3 --machine-type b2c.4x16
  ```
  {: pre}
...
```
{[bxcs]} worker-get --cluster [CLUSTER_NAME_OR_ID] --worker WORKER_NODE_ID [--json] [-s]
```
{: pre}
...
<strong>Command options</strong>:
   ...
   <dd>The name of your worker node. Run <code>{[bxcs]} workers <em>CLUSTER</em></code> to view the IDs for the worker nodes in a cluster. This value is required.</dd>
...
**Example**:
  ...
  ```
  {[bxcs]} worker-get --cluster my_cluster --worker kube-dal10-cr18a61a63a6a94b658596aa93d087aaa9-w1
  ```
  {: pre}
...
**Example output**:
  ...
  Public VLAN:  223xxxx
  Private IP:   {[internal_cluster_IP]}
  Public IP:    {[public_IP]}
  Hardware:     shared
  Zone:         dal10
  Version:      1.8.11_1509
...
Reboot a worker node in a cluster. During the reboot, the state of your worker node does not change. For example, you might use a reboot if the worker node status in {[softlayer]} is `Powered Off` and you need to turn on the worker node. A reboot clears temporary directories, but does not clear the entire filesystem or reformat the disks.
{: shortdesc}

```
{[bxcs]} worker-reboot [-f] [--hard] --cluster CLUSTER --worker WORKER [WORKER] [--skip-master-healthcheck] [-s]
```
{: pre}
...
1. List all worker nodes in your cluster and note the **name** of the worker node that you want to reboot.
   ...
   ```
   {: pre}
   The**name** that is returned in this command is the private IP address that is assigned to your worker node. You can find more information about your worker node when you run the `{[bxcs]} workers <cluster_name_or_ID>` command and look for the worker node with the same **Private IP** address.
...
3. Verify that pod scheduling is disabled for your worker node.
 ...
 5. Reboot the worker node. Use the worker ID that is returned from the `{[bxcs]} workers <cluster_name_or_ID>` command.
    ```
    {[bxcs]} worker-reboot --cluster <cluster_name_or_ID> --workers <worker_name_or_ID>
    ```
    {: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} worker-reboot --cluster my_cluster --worker kube-dal10-cr18a61a63a6a94b658596aa93d087aaa9-w1 kube-dal10-cr18a61a63a6a94b658596aa93d087aaa9-w2
  ```
  {: pre}
...
```
{[bxcs]} worker-reload [-f] --cluster CLUSTER --workers WORKER [WORKER] [--skip-master-healthcheck] [-s]
```
{: pre}
...
3. Verify that pod scheduling is disabled for your worker node.
 ...
 4. Force pods to be removed from your worker node and rescheduled onto remaining worker nodes in the cluster.
    ...
    {: pre}
    This process can take a few minutes.
 5. Reload the worker node. Use the worker ID that is returned from the `{[bxcs]} workers <cluster_name_or_ID>` command.
    ```
    {[bxcs]} worker-reload --cluster <cluster_name_or_ID> --workers <worker_name_or_ID>
    ```
    {: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} worker-reload --cluster my_cluster --workers kube-dal10-cr18a61a63a6a94b658596aa93d087aaa9-w1 kube-dal10-cr18a61a63a6a94b658596aa93d087aaa9-w2
  ```
  {: pre}
...
Remove one or more worker nodes from a cluster. If you remove a worker node, your cluster becomes unbalanced. You can automatically rebalance your worker pool by running the `{[bxcs]} worker-pool-rebalance` [command](#cs_rebalance).
{: shortdesc}

```
{[bxcs]} worker-rm [-f] --cluster CLUSTER --workers WORKER[,WORKER] [-s]
```
{: pre}
...
4. Force pods to be removed from your worker node and rescheduled onto remaining worker nodes in the cluster.
   ...
   {: pre}
   This process can take a few minutes.
5. Remove the worker node. Use the worker ID that is returned from the `{[bxcs]} workers <cluster_name_or_ID>` command.
   ```
   {[bxcs]} worker-rm --cluster <cluster_name_or_ID> --worker <worker_name_or_ID>
   ```
   {: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} worker-rm --cluster my_cluster --workers kube-dal10-cr18a61a63a6a94b658596aa93d087aaa9-w1 kube-dal10-cr18a61a63a6a94b658596aa93d087aaa9-w2
  ```
  {: pre}
...
Update worker nodes to apply the latest security updates and patches to the operating system, and to update the Kubernetes version to match the version of the Kubernetes master. You can update the master Kubernetes version with the `{[bxcs]} cluster-update` [command](/docs/containers?topic=containers-cs_cli_reference#cs_cluster_update).
{: shortdesc}

```
{[bxcs]} worker-update [-f] --cluster CLUSTER --workers WORKER[,WORKER] [--force-update] [-s]
```
{: pre}

<strong>Minimum required permissions</strong>: **Operator** platform role for the cluster in {{site.data.keyword.containerlong_notm}}

Running `{[bxcs]} worker-update` can cause downtime for your apps and services. During the update, all pods are rescheduled onto other worker nodes, the worker node is reimaged, and data is deleted if not stored outside the pod. To avoid downtime, [ensure that you have enough worker nodes to handle your workload while the selected worker nodes are updating](/docs/containers?topic=containers-update#worker_node).
{: important}
...
**Example**:
  ...
  ```
  {[bxcs]} worker-update --cluster my_cluster --worker kube-dal10-cr18a61a63a6a94b658596aa93d087aaa9-w1 kube-dal10-cr18a61a63a6a94b658596aa93d087aaa9-w2
  ```
  {: pre}
...
```
{[bxcs]} workers --cluster CLUSTER [--worker-pool POOL] [--show-pools] [--show-deleted] [--json] [-s]
```
{: pre}
...
<strong>Command options</strong>:
   ...
   <dd>View only worker nodes that belong to the worker pool. To list available worker pools, run `{[bxcs]} worker-pools --cluster <cluster_name_or_ID>`. This value is optional.</dd>
...
**Example**:
  ...
  ```
  {[bxcs]} workers --cluster my_cluster
  ```
  {: pre}
...
```
{[bxcs]} worker-pool-create --name POOL_NAME --cluster CLUSTER --machine-type MACHINE_TYPE --size-per-zone WORKERS_PER_ZONE --hardware ISOLATION [--labels LABELS] [--disable-disk-encrypt] [-s] [--json]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} worker-pool-create --name my_pool --cluster my_cluster --machine-type b2c.4x16 --size-per-zone 6
  ```
  {: pre}


### {[bxcs]} worker-pool-get
{: #cs_worker_pool_get}
...
```
{[bxcs]} worker-pool-get --worker-pool WORKER_POOL --cluster CLUSTER [-s] [--json]
```
{: pre}
...
<dl>
  <dt><code>--worker-pool <em>WORKER_POOL</em></code></dt>
    <dd>The name of the worker node pool that you want to view the details of. To list available worker pools, run `{[bxcs]} worker-pools --cluster <cluster_name_or_ID>`. This value is required.</dd>
  <dt><code>--cluster <em>CLUSTER</em></code></dt>
...
**Example**:
  ...
  ```
  {[bxcs]} worker-pool-get --worker-pool pool1 --cluster my_cluster
  ```
  {: pre}
...
```
{[bxcs]} worker-pool-rebalance --cluster CLUSTER --worker-pool WORKER_POOL [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} worker-pool-rebalance --cluster my_cluster --worker-pool my_pool
  ```
  {: pre}

### {[bxcs]} worker-pool-resize
...
```
{[bxcs]} worker-pool-resize --worker-pool WORKER_POOL --cluster CLUSTER --size-per-zone WORKERS_PER_ZONE [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} worker-pool-resize --cluster my_cluster --worker-pool my_pool --size-per-zone 3
  ```
  {: pre}
...
```
{[bxcs]} worker-pool-rm --worker-pool WORKER_POOL --cluster CLUSTER [-f] [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} worker-pool-rm --cluster my_cluster --worker-pool pool1
  ```
  {: pre}
...
```
{[bxcs]} worker-pools --cluster CLUSTER [--json] [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} worker-pools --cluster my_cluster
  ```
  {: pre}
...
```
{[bxcs]} zone-add --zone ZONE --cluster CLUSTER --worker-pools WORKER_POOL1[,WORKER_POOL2] --private-vlan PRIVATE_VLAN [--public-vlan PUBLIC_VLAN] [--private-only] [--json] [-s]
```
{: pre}
...
<dl>
  ...
  <dt><code>--private-vlan <em>PRIVATE_VLAN</em></code></dt>
    ...
    <p>If you have a private VLAN in the zone, this value must match the private VLAN ID of one or more of the worker nodes in the cluster. To see the VLANs that you have available, run <code>{[bxcs]} cluster-get --cluster &lt;cluster&gt; --showResources</code>. New worker nodes are added to the VLAN that you specify, but the VLANs for any existing worker nodes are not changed.</p>
    ...
    <p>{[enable_vlan_spanning]}</p></dd>

  <dt><code>--public-vlan <em>PUBLIC_VLAN</em></code></dt>
    <dd><p>The ID of the public VLAN. This value is required if you want to expose workloads on the nodes to the public after you create the cluster. It must match the public VLAN ID of one or more of the worker nodes in the cluster for the zone. To see the VLANs that you have available, run <code>{[bxcs]} cluster-get --cluster &lt;cluster&gt; --showResources</code>. New worker nodes are added to the VLAN that you specify, but the VLANs for any existing worker nodes are not changed.</p>
    <p>If you do not have a private or public VLAN in that zone, do not specify this option. A private and a public VLAN are automatically created for you when you initially add a new zone to your worker pool.</p>
    <p>{[enable_vlan_spanning]}</p></dd>
...
**Example**:
  ...
  ```
  {[bxcs]} zone-add --zone dal10 --cluster my_cluster --worker-pools pool1,pool2,pool3 --private-vlan 2294021
  ```
  {: pre}

### {[bxcs]} zone-network-set
{: #cs_zone_network_set}
...
```
{[bxcs]} zone-network-set --zone ZONE --cluster CLUSTER --worker-pools WORKER_POOL1[,WORKER_POOL2] --private-vlan PRIVATE_VLAN [--public-vlan PUBLIC_VLAN] [-f] [-s]
```
{: pre}
  ...
  {[matching_vlans]}
  <ol><li>Check the VLANs that are available in your cluster. <pre class="pre"><code>{[bxcs]} cluster-get --cluster &lt;cluster_name_or_ID&gt; --showResources</code></pre><p>Example output:</p>
  <pre class="screen"><code>Subnet VLANs
VLAN ID   Subnet CIDR         Public   User-managed
229xxxx   169.xx.xxx.xxx/29   true     false
229xxxx   10.xxx.xx.x/29      false    false</code></pre></li>
  <li>Check that the public and private VLAN IDs that you want to use are compatible. To be compatible, the <strong>Router</strong> must have the same pod ID.<pre class="pre"><code>{[bxcs]} vlans --zone &lt;zone&gt;</code></pre><p>Example output:</p>
...
229xxxx          5678     public    fcr01a.dal12   true</code></pre><p>Note that <strong>Router</strong> pod IDs match: `01a` and `01a`. If one pod ID were `01a` and the other were `02a`, you cannot set these public and private VLAN IDs for your worker pool.</p></li>
  ...
  **Example**:
  ...
  ```
  {[bxcs]} zone-network-set --zone dal10 --cluster my_cluster --worker-pools pool1,pool2,pool3 --private-vlan 2294021
  ```
  {: pre}
...
```
{[bxcs]} zone-rm --zone ZONE --cluster CLUSTER [-f] [-s]
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} zone-rm --zone dal10 --cluster my_cluster
  ```
  {: pre}
...
<img src="images/cs_cli_ref_imagemap_1.png" width="725px" usemap="#image-map" style="width:725px;"/>
<img src="images/cs_cli_ref_imagemap_2.png" width="725px" usemap="#image-map" style="width:725px;"/>
<img src="images/cs_cli_ref_imagemap_3.png" width="725px" usemap="#image-map" style="width:725px;"/>

## `apiserver` commands
{: #apiserver}

Use this group of commands to view and modify a cluster's API servers.
{: shortdesc}

### {[bxcs]} apiserver config
{: #cs_apiserver_config}

View and modify Kubernetes API server configuration options. This command must be combined with one of the following subcommands for a specific configuration option.
{: shortdesc}
...
Apply configuration changes for the Kubernetes master that are requested with the `{[bxcs]} apiserver-config-set`, `apiserver config unset`, `cluster feature enable`, or `cluster feature disable` commands. If a configuration change requires a restart, the affected Kubernetes master component is restarted. If the configuration changes can be applied without a restart, no Kubernetes master component is restarted. Your worker nodes, apps, and resources are not modified and continue to run.
{: shortdesc}
...
Use this group of commands to view, modify, and update cluster and cluster service settings.
{: shortdesc}
...
#### {[bxcs]} cluster addon disable debug-tool
...
```
{[bxcs]} cluster addon disable debug-tool --cluster CLUSTER
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster addon disable debug-tool --cluster my_cluster
  ```
  {: pre}
...
#### {[bxcs]} cluster addon disable kube-terminal
...
```
{[bxcs]} cluster addon disable kube-terminal --cluster CLUSTER
```
{: pre}
...
```
{[bxcs]} cluster addon disable kube-terminal --cluster my_cluster
```
{: pre}
...
#### {[bxcs]} cluster addon enable debug-tool
...
```
{[bxcs]} cluster addon enable debug-tool --cluster CLUSTER
```
{: pre}
...
**Example**:
  ...
  ```
  {[bxcs]} cluster addon enable debug-tool --cluster my_cluster
  ```
  {: pre}
...
#### {[bxcs]} cluster addon enable kube-terminal
...
```
{[bxcs]} cluster addon enable kube-terminal --cluster CLUSTER
```
{: pre}
...
```
{[bxcs]} cluster addon enable kube-terminal --cluster my_cluster
```
{: pre}

### {[bxcs]} cluster addon ls
{: #cs_cluster_addons}
...
```
{[bxcs]} cluster addon ls --cluster CLUSTER
```
{: pre}
...
```
{[bxcs]} cluster addon ls --cluster my_cluster
```
{: pre}

### {[bxcs]} cluster autoupdate
{: #cs_cluster_autoupdate}

Manage automatic patch updates of the cluster master. This command must be combined with one of the following subcommands.
{: shortdesc}

This group of commands is feature-flagged for select use cases only.
{: note}

#### {[bxcs]} cluster autoupdate disable
{: #cs_cluster_autoupdate_disable}

By default, automatic patch updates for the Kubernetes master are enabled. Use this command to disable automatic updates for the master. When automatic updates are disabled, you can manually update the master to the latest patch version by running the [`{[bxcs]} cluster update` command](/docs/containers?topic=containers-cs_cli_reference#cs_cluster_update).
{: shortdesc}

```
{[bxcs]} cluster autoupdate disable --cluster CLUSTER [--json]
```
{: pre}

Disabling automatic updates can help you verify patch changes in pre-production clusters before you allow production clusters' masters to be updated. However, the master version can never be behind the worker node version. For more information, see [Updating clusters, worker nodes, and add-ons](/docs/containers?topic=containers-update).
...
```
{[bxcs]} cluster autoupdate disable --cluster my_cluster
```
{: pre}

#### {[bxcs]} cluster autoupdate enable
{: #cs_cluster_autoupdate_enable}

If automatic updates of the Kubernetes master to the latest patch version are disabled, re-enable automatic updates. You can check whether automatic updates are disabled with the [`{[bxcs]} cluster autoupdate get` command](#cs_cluster_autoupdate_get). Whenever the [latest patch](/docs/containers?topic=containers-changelog) becomes available, the master is automatically updated to the latest patch.
{: shortdesc}

```
{[bxcs]} cluster autoupdate enable --cluster CLUSTER [--json]
```
{: pre}
...
```
{[bxcs]} cluster autoupdate enable --cluster my_cluster
```
{: pre}

#### {[bxcs]} cluster autoupdate get
...
```
 {[bxcs]} cluster autoupdate get --cluster CLUSTER [--json]
```
{: pre}
...
```
{[bxcs]} cluster autoupdate get --cluster my_cluster
```
{: pre}
...
```
{[bxcs]} cluster config --cluster CLUSTER [--admin] [--export] [--network] [-s] [--yaml]
```
{: pre}
...
<dt><code>--private-service-endpoint</code></dt>
  <dd>**Standard clusters in [VRF-enabled accounts](/docs/services/service-endpoint?topic=services/service-endpoint-getting-started#getting-started)**: Enable the [private service endpoint](/docs/containers?topic=containers-cs_network_ov#cs_network_ov_master_private) so that your Kubernetes master and the worker nodes communicate over the private VLAN. In addition, you can choose to enable the public service endpoint by using the `--public-service-endpoint` flag to access your cluster over the internet. If you enable the private service endpoint only, you must be connected to the private VLAN to communicate with your Kubernetes master. After you enable a private service endpoint, you cannot later disable it.<br><br>After you create the cluster, you can get the endpoint by running `{[bxcs]} cluster-get <cluster_name_or_ID>`.</dd>

<dt><code>--public-service-endpoint</code></dt>
  <dd>**Standard clusters in [VRF-enabled accounts](/docs/services/service-endpoint?topic=services/service-endpoint-getting-started#getting-started)**: Enable the [public service endpoint](/docs/containers?topic=containers-cs_network_ov#cs_network_ov_master_public) so that your Kubernetes master can be accessed over the public network, for example to run `kubectl` commands from your terminal. If you also include the `--private-service-endpoint` flag, [master-worker node communication](/docs/containers?topic=containers-cs_network_ov#cs_network_ov_master_both) is on the private network. You can later disable the public service endpoint if you want a private-only cluster.<br><br>After you create the cluster, you can get the endpoint by running `{[bxcs]} cluster-get <cluster_name_or_ID>`.</dd>
...
**Examples**:
  ...
  Example using the Container tribe {[softlayer]} account:

  ```
  {[bxcs]} cluster create --zone dal10 --public-vlan 1374057 --private-vlan 1374055 --name my_cluster
  ```
  {: pre}
...
### {[bxcs]} cluster feature disable public-service-endpoint
...
```
{[bxcs]} cluster feature disable public-service-endpoint --cluster CLUSTER [-s] [-f]
```
{: pre}
...
1. Enable the private service endpoint by running `{[bxcs]} cluster feature enable private-service-endpoint --cluster <cluster_name>`.
2. Run `{[bxcs]} apiserver-refresh --cluster <cluster_name>` to refresh the API server to use the service endpoint.
...
```
{[bxcs]} cluster feature disable public-service-endpoint --cluster my_cluster
```
{: pre}
...
#### {[bxcs]} cluster feature enable private-service-endpoint
...
```
{[bxcs]} cluster feature enable private-service-endpoint --cluster CLUSTER [-s]
```
{: pre}

To run this command:
1. Enable [VRF](/docs/infrastructure/direct-link?topic=direct-link-overview-of-virtual-routing-and-forwarding-vrf-on-ibm-cloud#overview-of-virtual-routing-and-forwarding-vrf-on-ibm-cloud) in your {[softlayer]} account.
...
3. Run `{[bxcs]} cluster feature enable private-service-endpoint --cluster <cluster_name>`.
4. Run `{[bxcs]} apiserver-refresh --cluster <cluster_name>` to refresh the API server to use the service endpoint.
...
```
{[bxcs]} cluster feature enable private-service-endpoint --cluster my_cluster
```
{: pre}

#### {[bxcs]} cluster feature enable public-service-endpoint
...
```
{[bxcs]} cluster feature enable public-service-endpoint --cluster CLUSTER [-s]
```
{: pre}

After you run this command, you must refresh the API server to use the service endpoint by running `{[bxcs]} apiserver-refresh --cluster <cluster_name>`.
...
```
{[bxcs]} cluster feature enable public-service-endpoint --cluster my_cluster
```
{: pre}


#### {[bxcs]} cluster feature enable trusted
...
```
{[bxcs]} cluster feature enable trusted --cluster CLUSTER [-s] [-f]
```
{: pre}

To check whether the bare metal machine type supports trust, check the **Trustable** field in the output of the `{[bxcs]} machine-types <zone>` [command](#cs_machine_types). To verify that a cluster is trust-enabled, view the **Trust ready** field in the output of the `{[bxcs]} cluster get` [command](#cs_cluster_get). To verify a bare metal worker node is trust-enabled, view the **Trust** field in the output of the `{[bxcs]} worker get` [command](#cs_worker_get).
...
```
{[bxcs]} cluster feature enable trusted --cluster my_cluster
```
{: pre}

### {[bxcs]} cluster feature ls
...
```
{[bxcs]} cluster feature ls --cluster CLUSTER [-s]
```
{: pre}
...
```
{[bxcs]} cluster feature ls --cluster my_cluster
```
{: pre}
...
### {[bxcs]} cluster service
{: #cs_cluster_service}

View, bind, and unbind {{site.data.keyword.Bluemix_notm}} services on a cluster. This command must be combined with one of the following subcommands.
{: shortdesc}

#### {[bxcs]} cluster service bind
{: #cs_cluster_service_bind}

Add an {{site.data.keyword.Bluemix_notm}} service to a cluster by binding the service instance to a Kubernetes namespace. To view available {{site.data.keyword.Bluemix_notm}} services from the {{site.data.keyword.Bluemix_notm}} catalog, run `{[bx]} service offerings`. **Note**: You can only add {{site.data.keyword.Bluemix_notm}} services that support service keys.
{: shortdesc}

```
{[bxcs]} cluster service bind --cluster CLUSTER --namespace KUBERNETES_NAMESPACE --service SERVICE_INSTANCE_NAME [--key SERVICE_INSTANCE_KEY] [--role IAM_ROLE] [-s]
```
{: pre}
...
```
{[bxcs]} cluster service ls --cluster CLUSTER [--namespace KUBERNETES_NAMESPACE] [--all-namespaces] [--json] [-s]
```
{: pre}
...
```
{[bxcs]} cluster service ls --cluster my_cluster --namespace my_namespace
```
{: pre}
...
<strong>Command options</strong>:
   ...
   <dt><code>--service <em>SERVICE_INSTANCE_GUID</em></code></dt>
   <dd>The ID of the {{site.data.keyword.Bluemix_notm}} service instance that you want to remove. To find the ID of the service instance, run `{[bxcs]} cluster service ls --cluster <cluster_name_or_ID>`.This value is required.</dd>
...
Add and create portable subnets for a cluster. This command must be combined with one of the following subcommands.
{: shortdesc}

#### {[bxcs]} cluster subnet add
{: #cs_cluster_subnet_add}

Make an existing portable public or private subnet in your {[softlayer]} account available to a cluster or reuse subnets from a deleted cluster instead of using the automatically provisioned subnets.
{: shortdesc}
...
Create a portable subnet in an {[softlayer]} account on your public or private VLAN and make it available to a cluster.
{: shortdesc}
...
<strong>Command options</strong>:
   ...
   <dl>
   <dt><code>--cluster <em>CLUSTER</em></code></dt>
   <dd>The name or ID of the cluster. This value is required. To list your clusters, use the `{[bxcs]} cluster ls` [command](#cs_clusters).</dd>

   <dt><code>--size <em>SIZE</em></code></dt>
   <dd>The number of subnet IP addresses. This value is required. Possible values are 8, 16, 32, or 64.</dd>

   <dd>The VLAN in which to create the subnet. This value is required. To list available VLANS, use the `{[bxcs]} vlan ls --zone <zone>` [command](#cs_vlans). The subnet is provisioned in the same zone that the VLAN is in.</dd>
...
### {[bxcs]} cluster user-subnet
{: #cs_cluster_user-subnet}

Add and remove user-managed portable subnets for a cluster. This command must be combined with one of the following subcommands.
{: shortdesc}
...
Use this group of commands to view, modify, and update worker nodes for a cluster.
{: shortdesc}

### Deprecated: {[bxcs]} worker add
{: #cs_worker_add}

Add standalone worker nodes to a cluster. This command is deprecated. Create a worker pool by running [`{[bxcs]} worker-pool create`](#cs_worker_pool_create) or add workers to an existing worker pool by running [`{[bxcs]} worker-pool resize`](#cs_worker_pool_resize).
{: deprecated}
...
<tr>
<td><code><em>zone</em></code></td>
<td>Replace <code><em>&lt;zone&gt;</em></code> with the zone to deploy your worker nodes. The available zones are dependent on the region that you are logged in. To list available zones, run <code>{[bxcs]} zone ls</code>.</td>
</tr>
<tr>
<td><code><em>machine-type</em></code></td>
<td>Replace <code><em>&lt;machine_type&gt;</em></code> with the type of machine that you want to deploy your worker nodes to. You can deploy your worker nodes as virtual machines on shared or dedicated hardware, or as physical machines on bare metal. Available physical and virtual machines types vary by the zone in which you deploy the cluster. For more information, see the `{[bxcs]} machine-types` [command](#cs_machine_types).</td>
</tr>
<tr>
<td><code><em>private-vlan</em></code></td>
<td>Replace <code><em>&lt;private_VLAN&gt;</em></code> with the ID of the private VLAN that you want to use for your worker nodes. To list available VLANs, run <code>{[bxcs]} vlan ls --zone <em>&lt;zone&gt;</em></code> and look for VLAN routers that start with <code>bcr</code> (back-end router).</td>
</tr>
<tr>
<td><code>public-vlan</code></td>
<td>Replace <code>&lt;public_VLAN&gt;</code> with the ID of the public VLAN that you want to use for your worker nodes. To list available VLANs, run <code>{[bxcs]} vlan ls --zone &lt;zone&gt;</code> and look for VLAN routers that start with <code>fcr</code> (front-end router). <br><strong>Note</strong>: {[private_vlan_vyatta]}</td>
</tr>
<tr>
<td><code>hardware</code></td>
<td>For virtual machine types: The level of hardware isolation for your worker node. Use dedicated if you want to have available physical resources dedicated to you only, or shared to allow physical resources to be shared with other IBM customers. The default is shared.</td>
...
<strong>Command options</strong>:
   ...
   <dt><code>--worker <em>WORKER_NODE_ID</em></code></dt>
   <dd>The name of your worker node. Run <code>{[bxcs]} worker ls --cluster <em>CLUSTER</em></code> to view the IDs for the worker nodes in a cluster. This value is required.</dd>
...
```
{[bxcs]} worker ls --cluster CLUSTER [--worker-pool POOL] [--show-pools] [--show-deleted] [--json] [-s]
```
{: pre}
...
<strong>Command options</strong>:
   ...
   <dt><code>--worker-pool <em>POOL</em></code></dt>
   <dd>View only worker nodes that belong to the worker pool. To list available worker pools, run `{[bxcs]} worker-pool ls --cluster <cluster_name_or_ID>`. This value is optional.</dd>
...
```
{[bxcs]} worker ls --cluster my_cluster
```
{: pre}
...
4. Force pods to be removed from your worker node and rescheduled onto remaining worker nodes in the cluster.
  ...
  {: pre}
  This process can take a few minutes.
5. Reboot the worker node. Use the worker ID that is returned from the `{[bxcs]} worker ls --cluster <cluster_name_or_ID>` command.
  ```
  {[bxcs]} worker reboot --cluster <cluster_name_or_ID> --workers <worker_name_or_ID>
  ```
  {: pre}
...
1. List all worker nodes in your cluster and note the **name** of the worker node that you want to reload.
   ...
   kubectl get nodes
   ```
   The **name** that is returned in this command is the private IP address that is assigned to your worker node. You can find more information about your worker node when you run the `{[bxcs]} worker ls --cluster <cluster_name_or_ID>` command and look for the worker node with the same **Private IP** address.
2. Mark the worker node as unschedulable in a process that is known as cordoning. When you cordon a worker node, you make it unavailable for future pod scheduling. Use the **name** of the worker node that you retrieved in the previous step.
   ```
   kubectl cordon <worker_name>
   ```
...
3. Verify that pod scheduling is disabled for your worker node.
 ...
 4. Force pods to be removed from your worker node and rescheduled onto remaining worker nodes in the cluster.
    ...
    {: pre}
    This process can take a few minutes.
 5. Reload the worker node. Use the worker ID that is returned from the `{[bxcs]} worker ls --cluster <cluster_name_or_ID>` command.
    ```
    {[bxcs]} worker reload --cluster <cluster_name_or_ID> --workers <worker_name_or_ID>
    ```
    {: pre}
...
4. Force pods to be removed from your worker node and rescheduled onto remaining worker nodes in the cluster.
   ...
   {: pre}
   This process can take a few minutes.
5. Remove the worker node. Use the worker ID that is returned from the `{[bxcs]} worker ls --cluster <cluster_name_or_ID>` command.
   ```
   {[bxcs]} worker rm --cluster <cluster_name_or_ID> --worker <worker_name_or_ID>
   ```
   {: pre}
6. Verify that the worker node is removed.
   ```
   {[bxcs]} worker ls --cluster <cluster_name_or_ID>
   ```
   {: pre}
</br>
<strong>Command options</strong>:
...
<dl>
  <dt><code>--worker-pool <em>WORKER_POOL</em></code></dt>
    <dd>The name of the worker node pool that you want to view the details of. To list available worker pools, run `{[bxcs]} worker-pool ls --cluster <cluster_name_or_ID>`. This value is required.</dd>
  <dt><code>--cluster <em>CLUSTER</em></code></dt>
    <dd>The name or ID of the cluster where the worker pool is located. This value is required.</dd>
...
```
{[bxcs]} worker-pool ls --cluster CLUSTER [--json] [-s]
```
{: pre}
...
```
{[bxcs]} worker-pool ls --cluster my_cluster
```
{: pre}
...
```
{[bxcs]} worker-pool rm --worker-pool WORKER_POOL --cluster CLUSTER [-s]
```
{: pre}
...
Use this group of commands to view and modify the zones attached to a worker pool.
{: shortdesc}
...
View a list of available zones that you can create a cluster in. The available zones vary by the region that you are logged in to. To switch regions, run `{[bxcs]} region set`.
{: shortdesc}

```
{[bxcs]} zone ls [--region-only] [--json] [-s]
```
{: pre}
...
229xxxx   169.xx.xxx.xxx/29   true     false
229xxxx   10.xxx.xx.x/29      false    false</code></pre></li>
<li>Check that the public and private VLAN IDs that you want to use are compatible. To be compatible, the <strong>Router</strong> must have the same pod ID.<pre class="pre"><code>{[bxcs]} vlan ls --zone &lt;zone&gt;</code></pre><p>Example output:</p>
<pre class="screen"><code>ID        Name   Number   Type      Router         Supports Virtual Workers
229xxxx          1234     private   bcr01a.dal12   true
229xxxx          5678     public    fcr01a.dal12   true</code></pre><p>Note that <strong>Router</strong> pod IDs match: `01a` and `01a`. If one pod ID were `01a` and the other were `02a`, you cannot set these public and private VLAN IDs for your worker pool.</p></li>
<li>If you do not have any VLANs available, you can <a href="/docs/infrastructure/vlans?topic=vlans-ordering-premium-vlans#ordering-premium-vlans">order new VLANs</a>.</li></ol>
...
Use this group of commands to view and configure an Ingress application load balancer (ALB).
{: shortdesc}

### {[bxcs]} alb autoupdate
{: #cs_alb_autoupdate}

Manage automatic updates for the Ingress ALB add-on in a cluster. This command must be combined with one of the following subcommands.
{: shortdesc}
...
Manage Ingress ALB certificates in a cluster. This command must be combined with one of the following subcommands.
{: shortdesc}
...
```
{[bxcs]} alb cert ls --cluster CLUSTER [--json] [-s]
```
{: pre}
...
```
{[bxcs]} alb cert ls --cluster my_cluster
```
{: pre}
...
```
{[bxcs]} alb configure --albID ALB_ID [--enable] [--user-ip USERIP] [--disable] [--disable-deployment] [-s]
```
{: pre}
...
List all Ingress ALB IDs in a cluster.
{: shortdesc}

```
{[bxcs]} alb ls --cluster CLUSTER [--json] [-s]
```
{: pre}
...
```
{[bxcs]} alb ls --cluster my_cluster
```
{: pre}
...
```
{[bxcs]} logging autoupdate disable --cluster mycluster
```
{: pre}
...
```
{[bxcs]} logging autoupdate enable --cluster mycluster
```
{: pre}
...
```
{[bxcs]} logging autoupdate get --cluster mycluster
```
{: pre}
...
```
{[bxcs]} logging filter get --cluster mycluster --id 885732 --show-matching-configs
```
{: pre}
...
```
{[bxcs]} logging refresh --cluster CLUSTER [--force-update] [-s]
```
{: pre}
...
**Example**:
```
{[bxcs]} logging refresh --cluster my_cluster
```
{: pre}
...
Use this group of commands to set and unset credentials that allow you to access the {[softlayer]} portfolio through your {{site.data.keyword.Bluemix_notm}} account.
{: shortdesc}

### {[bxcs]} credential get
{: #cs_credential_get}

If you set up your {{site.data.keyword.Bluemix_notm}} account to use different credentials to access the {[softlayer]} portfolio, get the infrastructure user name for the region and resource group that you are currently targeted to.
{: shortdesc}
...
Set credentials for a resource group and region that allow you to access the {[softlayer]} portfolio through your {{site.data.keyword.Bluemix_notm}} account.
{: shortdesc}
...
<dl>
  <dt><code>--infrastructure-username <em>USERNAME</em></code></dt>
  <dd>{[softlayer]} account API username. This value is required. Note that the infrastructure API username is not the same as the IBMid. To view the infrastructure API username:
  <ol>
  <li>Log in to the [{{site.data.keyword.Bluemix_notm}} console ![External link icon](../icons/launch-glyph.svg "External link icon")](https://cloud.ibm.com).
  <li>From the menu bar, select **Manage > Access (IAM)**.
  <li>Select the **Users** tab and then click on your user name.
...
Remove the credentials for a resource group and region that allow you to access the {[softlayer]} portfolio through your {{site.data.keyword.Bluemix_notm}} account.
{: shortdesc}

```
{[bxcs]} credential unset [-s]
```
{: pre}
...
List available portable subnets in your {[softlayer]} account.
{: shortdesc}
...
```
{[bxcs]} vlan ls --zone ZONE [--all] [--json] [-s]
```
{: pre}
...
View the VLAN spanning status for an {[softlayer]} account.
{: shortdesc}
...
**Example**:
```
{[bxcs]} init --host https://stage.cont.bluemix.net
```
{: pre}
...
For a list of available regions, review [regions and zones](/docs/containers?topic=containers-regions-and-zones) or use the `{[bxcs]} region ls` [command](#cs_regions).</dd></dl>
